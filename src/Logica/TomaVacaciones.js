document.addEventListener('DOMContentLoaded', async() => {
    ...
    async function loadEmployeeList() {
        ...
        try {
            ...
            // Base de la consulta que será común para todos los casos
            const baseQuery = `
                SELECT 
                ...
                LEFT JOIN (
                    SELECT 
                        IdPersonal,
                        SUM(DiasSolicitado) AS TotalDiasSolicitados
            ...
            // Definir la consulta según el puesto
            if (validarpuesto == 94) {
                query = `${baseQuery}
                    WHERE puestos.Id_PuestoGeneral IN (91, 143) 
                    AND personal.Estado IN (1, 2) 
                    AND departamentos.idivision = 2
                    ORDER BY departamentos.Nombre ASC`;
            } 
            else if (validarpuesto == 95) {
                query = `${baseQuery}
                    WHERE puestos.Id_PuestoGeneral IN (91, 143) 
                    AND personal.Estado IN (1, 2) 
                    AND departamentos.idivision = 3
                    ORDER BY departamentos.Nombre ASC`;
            } 
            else {
                query = `${baseQuery}
                    ...
                    AND personal.Estado IN (1,2) 
                    AND puestos.Id_PuestoGeneral != 91 
                    AND puestos.Id_PuestoGeneral != 143
                    ORDER BY DiasDisponibles DESC`;
                queryParams = [idDepartamento];
            }
            ...
            function renderEmployees(employees) {
                ...
                employees.forEach(employee => {
                    ...
                    const isCurrentUser = employee.IdPersonal === idPersonal;
                    
                    // Agregar clase especial si es el usuario actual
                    if (isCurrentUser) {
                    ...
                    // Solo agregar el event listener si no es el usuario actual
                    if (!isCurrentUser) {
                        ...
                        // Agregar un título que indique por qué no se puede seleccionar
                        row.title = 'No puedes seleccionarte a ti mismo';
